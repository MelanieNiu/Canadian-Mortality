...
R [@citeR] was the language and environment used for the bulk of this analysis, alongside the tidyverse [@thereferencecanbewhatever], arrow[@arrow], knitr[@knitr] , ggplot2[@Wickham2016ggplot2] have been used in data downloading, cleaning and visualization. 
...
cleaned_data |>
  filter(Year == 2021)
  ggplot(top_five, aes(x="", y=total_deaths, fill=cause_of_death)) +
...
Both models yield similar coefficients, suggesting they capture a similar relationship between causes and death counts. The Negative Binomial model, with a log likelihood of -1050.577, fits the data better than the Poisson model, which has a log likelihood of -16957.794. The negative binomial model's superior fit is further supported by a higher ELPD value compared to the Poisson model. Posterior predictive checks, trace plots, and Rhat plots assess MCMC convergence and the reliability of the Bayesian analysis, with details in @sec-model-details.
