...
```{r}
#| label: fig-2
#| fig-cap: Number of deaths from the top-five causes % of all deaths in 2021
#| echo: false
#| warning: false
#| message: false
...
# cleaned_data |>
#   filter(Year == 2021) |>
#   mutate(Percentage = Death / sum(Death) * 100) |>
#   ggplot(top_five, aes(x="", y= Death, fill= Cause)) +
...
coef_short_names_corrected <- c(
  ...
  "CauseCOVID-19" = "COVID-19"  # Ensure this matches the term exactly as it appears in your model output
)

# Apply the corrected mapping to rename terms for the Poisson model
tidy_poisson$term <- coef_short_names_corrected[tidy_poisson$term]
# Handling any terms that did not match by retaining their original names
tidy_poisson$term[is.na(tidy_poisson$term)] <- names(coef_short_names_corrected)[is.na(tidy_poisson$term)]

# Apply the corrected mapping to rename terms for the Negative Binomial model
...
# Assuming 'comparison' is your final data frame from previous steps
...
For instance, @tbl-3 shows that diseases of the heart have a coefficient of 1.454 in both models, suggesting the expected death count from this cause is approximately `r round(exp(1.454),2)`  times higher than from Accidents (unintentional injuries), after exponentiating the coefficient. Similarly, malignant neoplasms have a coefficient of approximately 1.805, indicating an expected count more than `r round(exp(1.805)-1, 2)` times higher than Accidents. Conversely, chronic lower respiratory diseases show a modest decrease in mortality compared to Accidents, with a coefficient of -0.070 in the Poisson model, indicating about `r round(1-exp(-0.070),2)` fewer deaths.

Table shows that among the top five leading causes of death in Canada, diseases of the heart with a coefficient of 1.454 and malignant neoplasms with a coefficient of approximately 1.805 significantly indicates a strong positive association in both models. The coefficient for Diseases of heart is 1.454 in both the Poisson model and the negative binomial model. Exponentiating the coefficient 1.454 to obtain suggests that the expected count of deaths due to Diseases of the heart is about `r round(exp(1.454) - 1,2)` times higher than the baseline category Accidents (unintentional injuries). Similarly, exponentiating the coefficient for malignant neoplasms to obtain `r round(exp(1.805),2)` shows the expected count of deaths is ` times higher Accidents (unintentional injuries). Conversely, chronic lower respiratory diseases are associated with a slight decrease in mortality compared to Accidents (unintentional injuries) with a coefficient of -0.070 in the Poisson model, although the impact is modest about  less. 

Both models yield similar coefficients, suggesting they capture a similar relationship between causes and death counts. The negative binomial model, with a log likelihood of -1050.577, fits the data better than the Poisson model, which has a log likelihood of -16957.794. The negative binomial model's better fit is further supported by a higher ELPD value compared to the Poisson model as shown in @tbl-4. Posterior predictive checks, trace plots, and Rhat plots assess MCMC convergence and the reliability of the Bayesian analysis, with details in @sec-model-details.
...
Future studies could aim to address these weaknesses by incorporating spatial and demographic variables to account for clustering effects. Future studies can broaden the scope to include more regions, socioeconomical variables, and their interactions. Longitudinal studies could offer insights into mortality trends and the long-term effects of environmental factors like climate change.
